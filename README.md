...
- **Anti-cheat (host-toggleable)** — because this is a P2P, host-authoritative game, a host-side guard authenticates the **origin** of every authoritative message (a client can't forge phase changes, votes, results, kicks, or chair status), validates **movement** (out-of-bounds clamping, speed/teleport detection), enforces **one country per peer** and **Security-Council vote eligibility**, checks **speaker/chat identity** (no impersonating another country), and **rate-limits** floods. The host sees blocked attempts live and can turn the whole system on/off.
